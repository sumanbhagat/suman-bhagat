...
try {
    ...
    if ($count == 0) {
        ...
        $stmt = $db->prepare("INSERT INTO hero_slides (title, subtitle, description, button1_text, button1_url, button2_text, button2_url, slide_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
    ...
    // Use default slide if database fails
    $slides = [
        [
